...
const PatientLogin = () => {
  ...
  const verifyPatientAccount = async (uid) => {
    try {
      const patientDoc = await getDoc(doc(db, 'patients', uid));
      
      if (!patientDoc.exists()) {
        return { valid: false, message: 'Patient account not found' };
      }
      
      const patientData = patientDoc.data();
      
      if (patientData.role !== 'patient') {
        return { valid: false, message: 'Account is not registered as a patient' };
      }
      
      return { valid: true, message: 'Success' };
    } catch (err) {
      return { valid: false, message: 'Error verifying account' };
    }
  };
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      const verification = await verifyPatientAccount(userCredential.user.uid);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const verification = await verifyPatientAccount(result.user.uid);
    ...
    <div className="patient-login-container">
      <div className="patient-login-card">
        ...
        <form onSubmit={handleEmailLogin}>
          ...
          <a href="/forgot-password?role=patient" className="patient-forgot-link">
          Forgot Password? 
          </a>
        ...
        <button className="patient-btn-google" onClick={handleGoogleLogin} disabled={loading}>
